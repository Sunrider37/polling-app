...
import com.example.quizapp.mapper.ModelMapper;
...
public class PollService {
    ...
    public PagedResponse<PollResponse> getAllPolls(UserPrincipal currentUser, int page, int size){
        ...
        List<PollResponse> pollResponses = polls.map(
                poll -> ModelMapper.mapPollToPollResponse(
                        poll,choiceVoteCountMap,creatorMap.get(poll.getCreatedBy()),
                        pollUserVoteMap == null ? null : pollUserVoteMap.getOrDefault(
                                poll.getId(),null
                        ))).getContent();
    ...
    public Poll createPoll(PollRequest pollRequest){
        ...
        pollRequest.getChoices().forEach(
                choiceRequest -> {poll.addChoice(new Choice(choiceRequest.getText()));}
        );
    ...
    public PollResponse pollPollById(Long pollId, UserPrincipal currentUser){
    ...
    public PollResponse castVoteAndGetUpdatedPoll(Long pollId, VoteRequest voteRequest,
        ...
        Choice selectedChoice = poll.getChoices().stream().filter(
                choice -> choice.getId().equals(voteRequest.getChoiceId())
        ).findFirst().orElseThrow(() -> new ResourceNotFoundException("Choice", "id",
        ...
        List<ChoiceVoteCount> votes = voteRepository.countByPollIdGroupByChoiceId(pollId);
        Map<Long, Long> choiceVoteMap = votes.stream().collect(Collectors.toMap(
        ...
        return ModelMapper.mapPollToPollResponse(poll,
                choiceVoteMap,creator,vote.getChoice().getId());
    }

    private Map<Long, User> getPollCreatorMap(List<Poll> polls) {
        List<Long> creatorIds = polls.stream()
                .map(Poll::getCreatedBy).distinct().collect(Collectors.toList());
    ...
    private Map<Long, Long> getPollUserVoteMap(UserPrincipal currentUser, List<Long> pollIds) {
        ...
        if(currentUser != null){
            List<Vote> userVotes = voteRepository.findByUserIdAndPollIdIn(
                    currentUser.getId(), pollIds
            );
